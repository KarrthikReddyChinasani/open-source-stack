...
function topics() {
  ...
  function success(topics) {
    return { type: topicsConstant.GET_TOPICS, topics };
  }
}

function favouriteTopics() {
  return dispatch => {
    topicsService.getFavouritetopics().then(
      topics => {
        dispatch(success(topics));
      },
      error => {
  ...
  function success(topics) {
    return { type: topicsConstant.GET_LIKED_TOPICS, topics };
  }
}

function likeTopic(id, isLiked) {
  topicsService.likeTopic(id, isLiked).then(
    topic => {
